...
import EventLanguageToggle from './EventLanguageToggle';
...
// Countdown component
const Countdown = ({ targetDate, lang = 'de' }: { targetDate: string; lang?: EventLanguage }) => {
  ...
  const daysLabel = lang === 'tr' ? 'Gün' : 'Tage';
  const hoursLabel = lang === 'tr' ? 'Saat' : 'Std';
  const minsLabel = lang === 'tr' ? 'Dk' : 'Min';
  ...
  return (
    <div className="flex gap-3 text-white">
      <div className="flex flex-col items-center bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/20">
        ...
        <span className="text-[8px] font-bold uppercase tracking-widest opacity-60">{daysLabel}</span>
      </div>
      <div className="flex flex-col items-center bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/20">
        <span className="text-lg font-black leading-none">{timeLeft.hours}</span>
        <span className="text-[8px] font-bold uppercase tracking-widest opacity-60">{hoursLabel}</span>
      </div>
      <div className="flex flex-col items-center bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/20">
        <span className="text-lg font-black leading-none">{timeLeft.minutes}</span>
        <span className="text-[8px] font-bold uppercase tracking-widest opacity-60">{minsLabel}</span>
      </div>
    </div>
  );
};
...
export default function Events() {
  const [lang, setLang] = useState<EventLanguage>(() => getSavedEventsLanguage());

  const handleLangChange = (newLang: EventLanguage) => {
    setLang(newLang);
    saveEventsLanguage(newLang);
  };

  const strings = eventUiStrings[lang];
    ...
    <section id="events" className="bg-white py-32 overflow-hidden">
      <div className="section-padding">
          ...
          <div className="max-w-2xl">
            <div className="flex items-center gap-4 mb-6 flex-wrap">
              <motion.div
                ...
                viewport={{ once: true }}
                className="inline-flex items-center gap-2 px-4 py-2 bg-brand-teal/10 rounded-full text-brand-teal text-xs font-black uppercase tracking-[0.2em]"
              >
                <Calendar size={14} /> {lang === 'tr' ? 'Gelecek Programlar' : 'Aktuelle Termine'}
              </motion.div>

              {/* Language toggle right in the header */}
              <EventLanguageToggle
                currentLang={lang}
                ...
                theme="light"
                showLabel={true}
                label={lang === 'tr' ? 'Dil' : 'Sprache'}
              />
            </div>

            <h2 className="text-4xl md:text-6xl font-extrabold text-brand-navy mb-6 leading-tight">
              {lang === 'tr' ? 'Öne Çıkan ' : 'Aktuelle '} 
              <span className="text-brand-teal">
                {lang === 'tr' ? 'Etkinlikler' : 'Veranstaltungen'}
              </span>
            </h2>
            <p className="text-slate-500 text-lg md:text-xl leading-relaxed">
              {lang === 'tr' 
                ? 'Gießen ve Wetzlar bölgesindeki yaklaşan etkinliklerimizi kaçırmayın. Kültürel buluşmalar ve eğitim programları sizleri bekliyor.'
                : 'Verpassen Sie keine unserer kommenden Veranstaltungen in Gießen und Wetzlar. Hier finden Sie die nächsten Highlights aus unserem Programm.'}
            </p>
          </div>
          <Link 
            to="/events"
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          {upcomingEvents.map((rawEvent, index) => {
            const event = getLocalizedEvent(rawEvent, lang)!;
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {(event.badge || (event as any).notice) && (
                        <div className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-lg ${
                          ...
                          {event.badge || (lang === 'tr' ? 'Ertelendi' : 'Verschoben')}
                        </div>
                      )}
                    </div>
                    {index === 0 && (
                      <div className="flex flex-col items-end gap-2">
                        <div className="flex items-center gap-2 text-white/60 text-[10px] font-black uppercase tracking-widest">
                          <Timer size={12} /> {strings.nextEventIn}
                        </div>
                        <Countdown targetDate={event.date} lang={lang} />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex flex-wrap gap-6 text-white/80 text-sm font-medium">
                      <div className="flex items-center gap-2">
                        ...
                        <span>{parseDateSafe(event.date).toLocaleDateString(dateLocale, { day: '2-digit', month: 'long' })}</span>
                      </div>
                      <div className="flex items-center gap-2">
                    ...
                    >
                      {isCancelled 
                        ...
                        : (event as any).requiresRegistration === false || event.badge?.toLowerCase().includes('ohne anmeldung') || event.badge?.toLowerCase().includes('kayıt gerekmez')
                          ? strings.detailsAndInfo 
                          : strings.detailsAndRegister} <ArrowRight size={18} />
                    </Link>
                  </div>
                </div>
              </motion.div>
