                ...
                <Card3D data-testid={`news-card-${n.id}`} className="h-full">
                  <div className="flex h-full flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-md">
                    <div className="relative h-52 overflow-hidden">
                      ...
                      <span className={`absolute left-4 top-4 rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-widest ${n.category === 'Vorschau' ? 'bg-brand-orange text-white' : 'bg-white/95 text-brand-teal'}`}>
                        {n.category}
                      </span>
                    </div>
                    <div className="flex flex-1 flex-col p-6">
                      ...
                      <p className="mt-3 flex-1 text-sm leading-relaxed text-slate-600">{n.shortText}</p>
                      <button
                        type="button"
                        data-testid={`news-more-${n.id}`}
                        onClick={() => setOpenNews(n)}
                        className="mt-4 inline-flex w-fit items-center gap-1.5 rounded-full bg-brand-teal px-5 py-2.5 text-sm font-bold text-white transition-all hover:gap-2.5 hover:bg-teal-600 active:scale-95"
                      >
            ...
            {feeds.events.map((e, i) => (
              ...
              >
                <Card3D data-testid={`event-card-${e.id}`} className="h-full">
                  <Link to={e.link} className="flex h-full flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-md">
                    <div className="relative h-44 overflow-hidden">
                      <img src={e.image} alt={e.title} loading="lazy" className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                      <span className="absolute left-4 top-4 rounded-full bg-white/95 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-brand-orange">
                        {e.category}
                      </span>
                    </div>
                    <div className="flex flex-1 flex-col p-5">
                      <time className="font-mono text-[11px] uppercase tracking-wide text-slate-400">{formatDate(e.date)}</time>
                      <h3 className="mt-1 text-lg font-extrabold leading-snug text-brand-navy">{e.title}</h3>
                      <p className="mt-1 inline-flex items-center gap-1.5 text-xs text-slate-500">
          ...
          >
            ...
            >
              <div className="relative h-56">
                <img src={openNews.image} alt={openNews.title} className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                ...
                <button
                  ...
                  className="absolute right-4 top-4 rounded-full bg-white/90 p-2 text-slate-700 transition-all hover:bg-white active:scale-95"
                >
                  <X size={18} />
                </button>
                <div className="absolute bottom-4 left-6 right-6">
                  <time className="font-mono text-[11px] uppercase tracking-wide text-white/80">{formatDate(openNews.date)}</time>
                  <h3 className="text-2xl font-extrabold leading-tight text-white">{openNews.title}</h3>
                </div>
              </div>

              <div className="max-h-[55vh] overflow-y-auto p-6 sm:p-8">
                {openNews.location && (
                  <p className="mb-4 inline-flex items-center gap-1.5 text-sm text-slate-500">
                    <MapPin size={14} className="text-brand-teal" /> {openNews.location}
                  </p>
                )}
                <p className="text-slate-700">{openNews.shortText}</p>
                ...
                {openNews.speech && (
                  <div className="mt-6 rounded-2xl border-l-4 border-brand-teal bg-brand-teal/5 p-5">
                    <Quote size={18} className="text-brand-teal" />
